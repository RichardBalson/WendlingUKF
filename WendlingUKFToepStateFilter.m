...
if filter_simulation
    ...
    filter_length =length(band_coeff);
    band_coeffT = [band_coeff zeros(1,(filter_length-1)/2)];
    tope = toeplitz(band_coeffT,zeros((filter_length-1)*1.5+1,1));
    topeF = tope(end,:);
    filter_length = length(band_coeffT);
end
...
for q = 1:Simulation_number
    ...
    while condition
        ...
        for p =1:Number_of_observations
            ...
            if (filter_simulation)
                if p> filter_length
                Test(:,p) = (topeF*Test(:,p-filter_length:p-1)')';
                ExpX(1:Ds,p) = Test(:,p);
                else
                    Extension = ones(filter_length-p,Ds);
                    Test(:,p) = topeF*[Extension;ExpX(1:Ds,1:p)'];
                        end
            end
