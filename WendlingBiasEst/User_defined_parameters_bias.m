...
if filter_simulation
    highcutoff = 1.5;%0.5;%2.5; % Specify highcutoff frequency for filter
    
    lowcutoff = 250;%80;%40; % Specify low cutoff frequency for filter
...
NoiseIn = 1e0;%1e-3;% Base 1e-2 Specify noise to add to simulated signal
...
if simulate % Specify parameters for simulation purposes
    SimulationSettings.simulation_time =10; %Time for simulation in seconds 
    ...
    if SimulationSettings.Parameter_index % Specify synaptic gains for simulation
        SimulationSettings.AV= [4 4 4 4 5 5 6 6];%[5 5 5 5 5 7 5];%[4 4 4 4 5 5 6 6];
        SimulationSettings.BV= [15 15 15 25 25 30 30 30];%[20 20 20 30 20 20 20];%[15 15 15 25 25 30 30 30];
        SimulationSettings.GV= [30 30 30 25 25 20 20 20];%[20 30 20 20 20 20 20];%[30 30 30 25 25 20 20 20];
    end
...
Do = 1; % Number of parameters to estimate for observation function
...
Observation_uncertainty = 1e1; % Specify the uncertainty in observations

Bias_uncertainty = 2e-2;
...
Image_handling_model_output=[1;1];
...
Image_handling_multi = [1 1 1;1 0 0];%  % Here a decision is made whether to plot specific states, 
                                                                            % if the value is one the relevant figure is plotted, otherwise it is not.
...
Estimation_Error = 0; % Plt the error on each estimate on a single plot
