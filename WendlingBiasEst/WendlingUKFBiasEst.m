...
for q = 1:Simulation_number
    ...
    % ~~~~~~~~~~~~~~~~
    output8 = output8/Data_scale+DC_bias;
    if filter_simulation
    ...
    while condition
        ...
        for p =1:Number_of_observations
            ...
            if Do ==1
                Bias=Sigma(Ds+Dk+Dp+1,:,p);
            else
                Bias = DC_bias*ones(1,size(Sigma,2));
            end
                
            
            [Xout(:,:,p) Yout(:,:,p)] = WNMBias(Sigma(:,:,p),dt,Input_var, gain, tcon,C,Bias);
