...
if (Parameter_initialisation ==0)
    ...
    mu_scale =(ScaleR(2)+ScaleR(1))/2;
    mu_bias =(BiasR(2)+BiasR(1))/2;
    ...
    sigma_Scale = (ScaleR(2)-ScaleR(1))/(2*number_of_sigma);
    sigma_Bias = (BiasR(2)-BiasR(1))/(2*number_of_sigma);
    conditionP =1;
    while conditionP
    ...
    conditionS =1;
    while conditionS
        Initial_scale = mu_scale + randn(1)*sigma_Scale;
        Initial_bias = mu_bias +randn(1)*sigma_Bias;
        conditionS = Initial_scale<0;
    end
...
if Do>0
    Parameter_std_deviation(1,Dp+1) = max(ScaleR(2)-Initial_scale,Initial_scale-ScaleR(1))/number_of_sigma*std_adjustment_parameters;
    if Do>1
        Parameter_std_deviation(1,Dp+2) = max(BiasR(2)-Initial_bias,Initial_bias-BiasR(1))/number_of_sigma*std_adjustment_parameters;
    end       
end
...
Parameter_std_deviationF = repmat(Parameter_std_deviation,Dp+Do,1);% Define a parameter variance matrix that can easily be used for future purposes.
...
if Dp >0                                    % Check whether parameters need to be estimated, if so alter storage folder
    ...
    Xp(1,1) = Initial_excitability;     % Set the intial guess for the A to be the initial value specified.
    if Dp >1                                % Check whether more than one parameter needs to be estimated
        Xp(2,1) = Initial_inhibition;    % Set the intial guess for the B to be the initial value specified.
        if Dp==3                            % Check if parameter G needs to be estimated
            Xp(3,1) = Initial_Finhibition;% Set the intial guess for the G to be the initial value specified.
        end
    end
else
...
if Do >0
    Xp(Dp+1,1) = Initial_scale;
    if Do>1
        Xp(Dp+2,1) = Initial_bias;
    end
end

Parameter_covariance_matrix = eye(Dp+Do).*Parameter_std_deviationF.^2; % State Parameter matrix
