...
if Do>0
    mean_state(1,Ds) = mean_state(1,2)-C(4)*mean_state(1,3)-mean_state(1,4);
    Base_state_uncertainty(1,Ds) = 1e-6;
    State_std_deviation(1,Ds) = sqrt(Base_state_uncertainty(1,Ds));
end
...
State_uncertainty = Base_state_uncertainty; % Specify base
...
if Do >0
    Parameter_uncertainty(1,Dp+1) = Scale_uncertainty; % Variance to allow for model error and stochastic input effect
    if Do >1
        Parameter_uncertainty(1,Dp+2) = Bias_uncertainty;
    end
else
    Parameter_uncertainty = [];
end
...
Parameter_uncertaintyF = repmat(Parameter_uncertainty,Dp+Do,1);% Define a parameter uncertainty matrix that can easily be used for future purposes.
...
Parameter_uncertainty_matrix = eye(Dp+Do).*Parameter_uncertaintyF.^2.*uncertainty_adjustment;
