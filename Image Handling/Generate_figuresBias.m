...
index = {1:length(check) tstart*sampling_frequency+1:(tstart+zoomtime)*sampling_frequency+1 ...
    EstStart_Sample:length(output8) round((EstStart+tstart)*sampling_frequency)+1:round((EstStart+tstart+zoomtime)*sampling_frequency)+1};
...
if (Image_handling_multi(1,1) ==1) % Plot all model states
    RowP = floor(Ds/2); % Set number of rows for multi plot
...
if (Image_handling_multi(1,2) ==1) % Plot all state inputs
    ...
    input = [Y(index{1})';ExpY(index{1});...
            ...
            (C(5)*z(index{3},1)-C(6)*z(index{3},3))';C(5)*X(1,index{1})-C(6)*X(3,index{1})];
...
if ((Image_handling_multi(1,3) ==1) &&(Dp+Dk >0)) % Plot model parameters estimated
    ...
    NMM(3) = state_figure_multi(fig_name,'State',fig_settings,t,input,{'Sim.','Est.','Std. Dev.'},[erfn;erfp],RowP,ColP,namesS,PlotsPerFig);
    clear input erfn erfp namesS    
...
if ((Image_handling_multi(2,3) ==1) &&(Dp+Dk >0)) % Plot model parameters estimated
    ...
    if Dk 
        ...
        fig_name = 'Neural Mass Parameters';
    namesS = {'G_{p} (mV)','G_{s} (mV)','G_{f} (mV)','Input (Hz)'};
    NMM(3) = state_figure_multi(fig_name,'State',fig_settings,tz,input,{'Sim.','Est.','Std. Dev.'},[erfn;erfp],RowP,ColP,namesS,PlotsPerFig);
    clear input erfn erfp namesS  
end
...
if (Image_handling_firing_rates(4) ==1)
    ...
    ColP=1;
    PlotsPerFig =2;
        input = [(C(5)*z(index{3},1)-C(6)*z(index{3},3))';C(5)*X(1,index{1})-C(6)*X(3,index{1});...
            Sigmoid(C(5)*z(index{3},1)-C(6)*z(index{3},3))';Sigmoid(C(5)*X(1,index{1})-C(6)*X(3,index{1}));...
            z(index{3},4)';X(4,index{1})];
...
if (Estimation_Error && (Dp+Dk>0))
    ...
    namesS = {'G_{p} (% Error)','G_{s} (% Error)','G_{f} (% Error)','Input (% Error)'};
...
if Do>0
    ...
    PlotsPerFig =2;
    for k =1:Do
    input((k-1)*PlotsPerFig+1:k*PlotsPerFig,:) = [inD(k,:)*ones(1,length(index{2}));X(Ds+Dk+Dp+k,index{2})];
        if plot_uncertainty
          erfn(k,:) = X(Ds+Dk+Dp+k,index{2})-squeeze(sqrt(Pxx(Ds+Dk+Dp+k,Ds+Dk+Dp+k,index{2})*Error_multiplier))';
          erfp(k,:) = X(Ds+Dk+Dp+k,index{2})+squeeze(sqrt(Pxx(Ds+Dk+Dp+k,Ds+Dk+Dp+k,index{2})*Error_multiplier))';
        else erfn =[]; erfp =[];
        end
    end
        fig_name = 'Neurl Mass Observation Function';
    namesS = {'Scale','Bias'};
    NMSPZ(1) = state_figure_multi(fig_name,'State',fig_settings,tz,input,{'Sim.','Est.','Std. Dev.'},[erfn;erfp],RowP,ColP,namesS,PlotsPerFig);
...
if Do>0
    ...
    PlotsPerFig =2;
    for k =1:Do
    input((k-1)*PlotsPerFig+1:k*PlotsPerFig,:) = [inD(k,:)*ones(1,length(index{1}));X(Ds+Dk+Dp+k,index{1})];
        if plot_uncertainty
          erfn(k,:) = X(Ds+Dk+Dp+k,index{1})-squeeze(sqrt(Pxx(Ds+Dk+Dp+k,Ds+Dk+Dp+k,index{1})*Error_multiplier))';
          erfp(k,:) = X(Ds+Dk+Dp+k,index{1})+squeeze(sqrt(Pxx(Ds+Dk+Dp+k,Ds+Dk+Dp+k,index{1})*Error_multiplier))';
        else erfn =[]; erfp =[];
        end
    end
