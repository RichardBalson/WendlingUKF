...
index = {1:size(X_Multi,1) (EstStart)*sampling_frequency+1:Decimate:length(output8)};
...
if Dp>0
    for k = 1:Dp
        if (Image_handling_states(1,Ds+k+1))
            ...
            NMSM(Dk+k-1)=state_figure(fig_name{k},'Multi',fig_settings,t,[MVI(index{2},k)';squeeze(X_Multi(:,Dk+k,:))],{'Sim. Input','Est. Input','Std. Dev.'},[erfn;erfp]);
        end
    end
end
