...
index = {1:size(X_Multi,1) 1:Decimate:length(Y)};
...
if Dk
    if (Image_handling_states(1,9))
        ...
        NMSM(Dk)=state_figureInput('Neural Mass (Input)','State',fig_settings,t,[squeeze(X_Multi(index{1},Ds+Dk,:))],{'Est. Input','Std. Dev.'},[erfn;erfp]);
...
if Dp>0
    for k = 1:Dp
        if (Image_handling_states(1,Ds+k+1))
            ...
            NMSM(Dk+k-1)=state_figure(fig_name{k},'State',fig_settings,t,[squeeze(X_Multi(:,Dk+k,:))],{'Est. Input','Std. Dev.'},[erfn;erfp]);
        end
    end
end
...
if ((Image_handling_multi(1,3) ==1) &&(Dp+Dk >0)) % Plot model parameters estimated
    ...
    PlotsPerFig =Simulation_number;
    for k =1:Dp
        input(:,:,k) = [squeeze(X_Multi(:,Dk+k,:))'];
    ...
    NMMM = state_figure_multi_sim(fig_name,'State',fig_settings,t,input,{'Est.'},erfn,erfp,RowP,ColP,namesS,PlotsPerFig);
