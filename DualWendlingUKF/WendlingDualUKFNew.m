...
%             [Xexp Yexp] = WNM(X(:,p),dt,Input_var, gain, tcon,C);
            ...
            % Using estimated states as observations

            [XoutP(:,:,p),~] = WNM1(ExpX(:,p),dt,Input_var, SigmaP(:,:,p), tcon,C);
            
            [ExpX(:,p) ExpY(:,p) PxxPn PxyPn PyyPn] = Expectation1(XoutP(:,:,p), Ds, X(:,p+1), Ds,kappa);
