...
if kappa > 0
    Sigma_points = 2*Ds+1;  % Specify number of sigma points to generate, if kappa is greater than zero than the mean is propagated
    % through the system and there is therefore one more sigma point generated
else
    Sigma_points = 2*Ds; % When kappa is set to zero sigma points are generated, note there are twice the number of sigma points as there are states
...
X = zeros(Ds,Number_of_observations); % Intialise state estimate matrix
Xp = zeros(Dp,Number_of_observations);
Pxx = zeros(Ds,Ds,Number_of_observations);% Intialise state covariance estimate matrix
PxxP = zeros(Dp,Dp,Number_of_observations); % Initiliase parameter covariance matrix
Xout = zeros(Ds,Sigma_points,Number_of_observations); % Intialise output states from the point sin the unscented transform matrix
Yout = zeros(Dy,Sigma_points,Number_of_observations); % Intialsie the model output for each set of sigm points matrix
ExpX = zeros(Ds,Number_of_observations);% Intialise athe expected states from the unscented transform matrix
ExpY = zeros(Dy,Number_of_observations);% Intialise the expected states from the unscented transform matrix
Sigma = zeros(Ds,Sigma_points,Number_of_observations);% Intialise the sigma points matrix
...
Q = State_uncertainty_matrix;

Qp = Parameter_uncertainty_matrix;
