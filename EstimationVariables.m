...
if ((min(MVI(:,1))==max(MVI(:,1))) && (min(MVI(:,2))==max(MVI(:,2))) && (min(MVI(:,3))==max(MVI(:,3))))
    ...
    simulation_initial_name = [Estimation_Type,'f',int2str(sampling_frequency),...
    ...
    Parameter_varying=1;
    simulation_initial_name = [Estimation_Type,'V_f=',int2str(sampling_frequency),...
        '_A,B,G_',int2str(MVI(1,1)),',',int2str(MVI(1,2)),',',int2str(MVI(1,3)),'S',num2str(SimulationSettings.stochastic)]; % Initaite name for simulation, used for saving purposes
end
