...
index = {1:length(Y) tstart*sampling_frequency+1:(tstart+zoomtime)*sampling_frequency+1};
...
if (Image_handling_model_output(2,1))
    ...
    legendT = {'Obs.','Est.'};
    NMO=state_figure('Wendling Neural Mass Output zoomtimeed In','Obs',fig_settings,tz,[Y(index{2})'; X(2,index{2})-X(3,index{2})],legendT,[]);
...
if (Image_handling_multi(1,1) ==1) % Plot all model states
    ...
    for k = 1:Ds
        ...
        if plot_uncertainty
          erfn(k,:) = X(k,index{1})-(sqrt(Pxx(k,index{1})*Error_multiplier));
          erfp(k,:) = X(k,index{1})+(sqrt(Pxx(k,index{1})*Error_multiplier));
        else erfn =[]; erfp =[];
        end
    end
...
if (Image_handling_multi(2,1) ==1) % Plot all model states
    ...
    for k = 1:Ds
        ...
        if plot_uncertainty
          erfn(k,:) = X(k,index{2})-(sqrt(Pxx(k,index{2})*Error_multiplier));
          erfp(k,:) = X(k,index{2})+(sqrt(Pxx(k,index{2})*Error_multiplier));
        else erfn =[]; erfp =[];
        end
    end
    namesS = {'v_{po} (mV)','v_{eo} (mV)','v_{sio} (mV)','v_{fio} (mV)' 'Z_{po} (mV)','Z_{eo} (mV)','Z_{sio} (mV)','Z_{fio} (mV)'};
...
if ((Image_handling_multi(1,3) ==1) &&(Dp+Dk >0)) % Plot model parameters estimated
    ...
    input((k-1)*PlotsPerFig+1:k*PlotsPerFig,:) = [X(Ds+Dk+k,index{1})];
        if plot_uncertainty
          erfn(k,:) = X(Ds+Dk+k,index{1})-(sqrt(Pxx(Ds+Dk+k,index{1})*Error_multiplier));
          erfp(k,:) = X(Ds+Dk+k,index{1})+(sqrt(Pxx(Ds+Dk+k,index{1})*Error_multiplier));
        else erfn =[]; erfp =[];
        end
    end
    if Dk 
        ...
        if plot_uncertainty
          erfn(Dp+1,:) = X(Ds+1,index{1})-(sqrt(Pxx(Ds+1,index{1})*Error_multiplier));
          erfp(Dp+1,:) = X(Ds+1,index{1})+(sqrt(Pxx(Ds+1,index{1})*Error_multiplier));
          else erfn =[]; erfp =[];
        end
    end
...
if (Image_handling_multi(2,3) ==1) % Plot model parameters estimated
    ...
    input((k-1)*PlotsPerFig+1:k*PlotsPerFig,:) = [X(Ds+Dk+k,index{2})];
        if plot_uncertainty
          erfn(k,:) = X(Ds+Dk+k,index{2})-(sqrt(Pxx(Ds+Dk+k,index{2})*Error_multiplier));
          erfp(k,:) = X(Ds+Dk+k,index{2})+(sqrt(Pxx(Ds+Dk+k,index{2})*Error_multiplier));
        else erfn =[]; erfp =[];
        end
    end
    if Dk 
        ...
        if plot_uncertainty
          erfn(Dp+1,:) = X(Ds+1,index{2})-(sqrt(Pxx(Ds+1,index{2})*Error_multiplier));
          erfp(Dp+1,:) = X(Ds+1,index{2})+(sqrt(Pxx(Ds+1,index{2})*Error_multiplier));
          else erfn =[]; erfp =[];
        end
    end
        fig_name = 'Neural Mass Parameters';
