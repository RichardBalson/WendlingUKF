...
if Dp >0
    Parameter_uncertainty(1,1) = Exc_parameter_uncertainty + Variable_parameter_uncertainty; % Variance to allow for model error and stochastic input effect
    if Dp >1
        Parameter_uncertainty(1,2) = (SInh_parameter_uncertainty + Variable_parameter_uncertainty);
        if Dp >2
            Parameter_uncertainty(1,3) = (FInh_parameter_uncertainty + Variable_parameter_uncertainty); %[m*1e-1+1e-3 m*2.5e-1+1e-3 m*2.5e-1+1e-3]
        end
    end
else
