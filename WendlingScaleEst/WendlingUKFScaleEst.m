function [X Pxx X_Multi Pxx_Multi]= WendlingUKF
...
Ds = Ds + (Do>0);
Dx = Ds+Dp+Dk+Do; % Number of dimensions of augmented state matrix, Note that estimated parameters and inputs are now considered to be 'slow states' in the estimation procedure
...
for q = 1:Simulation_number
    ...
    while condition
        ...
        for p =1:Number_of_observations
            ...
            elseif Dp ==2
                gain = [Sigma(Ds+Dk+1,:,p); Sigma(Ds+Dk+2,:,p); ones(1,size(Sigma,2))*MVI(EstStart_Sample+p-1,3)];
            elseif Dp==1
                gain = [Sigma(Ds+Dk+1,:,p); ones(1,size(Sigma,2))*MVI(EstStart_Sample+p-1,2); ones(1,size(Sigma,2))*MVI(EstStart_Sample+p-1,3)];
            else
                gain = [ones(1,size(Sigma,2))*MVI(EstStart_Sample+p-1,1); ones(1,size(Sigma,2))*MVI(EstStart_Sample+p-1,2); ones(1,size(Sigma,2))*MVI(EstStart_Sample+p-1,3)];
            end
            ...
            if Do ==1
                Scale= [Sigma(Ds+Dk+Dp+1,:,p);zeros(1,size(Sigma,2))]; 
            elseif Do==2
                Scale=[Sigma(Ds+Dk+Dp+1,:,p);Sigma(Ds+Dk+Dp+2,:,p)];
            else
                Scale = [Data_scale*ones(1,size(Sigma,2));zeros(1,size(Sigma,2))];
            end
                
            
            [Xout(:,:,p) Yout(:,:,p)] = WNMScale(Sigma(:,:,p),dt,Input_var, gain, tcon,C,Scale);
